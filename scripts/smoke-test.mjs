...
async function extract(url, label, model, schema = { type: "object", properties: { appName: { type: ["string", "null"] }, loginUser: { type: ["string", "null"] }, errorMessage: { type: ["string", "null"] } } }) { const body = { imageUrl: url, prompt: "Dùng OCR trích toàn bộ thông tin trên hình", model, parameters: { max_tokens: 4096, temperature: 0.2 }, output: { includeRawText: true, includeAnnotations: true, schema }, metadata: { source: label } }; const headers = { "content-type": "application/json" }; if (apiKey) headers["x-api-key"] = apiKey; return readJson(await fetch(`${workerUrl}/v1/extract`, { method: "POST", headers, body: JSON.stringify(body) }), label); }
