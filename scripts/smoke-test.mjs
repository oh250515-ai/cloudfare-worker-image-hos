...
async function extract(overrides, label) {
  ...
  Object.assign(request, overrides, {
    prompt: "Smoke test: describe the image and preserve every visible character in rawText. Return structured JSON without guessing.",
...
async function readableBase64Probe() {
  const response = await fetch(SAFE_TEXT_IMAGE_URL);
  if (!response.ok) fail(`Could not download safe smoke image, HTTP ${response.status}`);
  const mimeType = response.headers.get("content-type")?.split(";")[0] || "image/png";
  const imageBase64 = Buffer.from(await response.arrayBuffer()).toString("base64");
  return { imageBase64, imageMimeType: mimeType };
}
...
try {
  ...
  if (imageUrl) {
    ...
    catch (error) { console.warn(`::warning title=Configured test image failed::${safeMessage(error instanceof Error ? error.message : error)}; retrying with readable base64 probe`); }
  }
